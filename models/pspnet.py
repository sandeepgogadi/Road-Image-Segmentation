...
def conv_block(input_tensor, filters, strides, d_rates):
    ...
    x = Activation('relu')(x)
    x = Conv2D(filters[1], kernel_size=3, strides=strides,
               padding='same', dilation_rate=d_rates[1])(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
...
def pyramid_pooling_block(input_tensor, bin_sizes):
    ...
    for bin_size in bin_sizes:
        x = AveragePooling2D(pool_size=(h//bin_size, w//bin_size),
...
def PSPNET(width, height, num_classes, weights_path=None, train=False):
